...
## Fase 7 — ProjectsGallery (hecho)

- `useHorizontalGallery` (nuevo hook): puerto literal del bloque `horizontalSection` de `main.js` (medición/render/reset, `matchMedia('(min-width:901px)')`, `ResizeObserver`, listeners de scroll/resize/orientationchange/load, `document.fonts.ready`). Conserva el orden exacto de escritura-antes-que-lectura del original (`track.style.transform` se fija antes de leer `getBoundingClientRect()` de cada tarjeta, forzando el reflow síncrono que refleja el transform del frame actual) — condición necesaria para que el zoom/proximidad de cada tarjeta sea correcto en el mismo frame, no con un frame de retraso. Como todo hook de `motion/`, no toca `useState`/React: lee/escribe el DOM directamente y expone limpieza completa de listeners/`ResizeObserver`/`requestAnimationFrame` en el efecto (adaptación de ciclo de vida necesaria en React, no presente en el script original de página única).
- `ProjectsGallery`: puerto literal de la sección `.projects` (7 tarjetas con imagen/categoría/número/título en dos líneas, barra de progreso y contador). Reutiliza `SectionLabel` (fase 5) y el atributo `data-cursor="Abrir"` ya soportado por `useCustomCursor` (fase 2) sin tocar ese hook. Selectores estables entre CSS Modules y hook vía atributos `data-*`: `data-horizontal-section`/`data-horizontal-track` (nombres tomados literalmente del original), `data-horizontal-viewport`, `data-project-card`, `data-project-progress-bar` (nuevos) y `data-project-current` (también literal del original).
- **Validación del recorrido completo, no de un único estado**: a diferencia de las secciones anteriores (una sola captura "en reposo"), aquí el oráculo y los tests cubren 0/25/50/75/100% del recorrido en cada viewport de escritorio, calculando la posición de scroll con la misma fórmula que usa el propio hook (`horizontalStart + progreso × horizontalDistance`) para no depender de asunciones aproximadas. En viewports ≤900px la sección cae a un grid estático de 1-2 columnas sin sticky ni transform (mismo criterio que Manifesto/Solutions): una sola captura basta.
- **Comportamiento verificado explícitamente** (más allá de paridad de píxel): estado inicial (tarjeta 1, sin desplazamiento), inicio del sticky, llegada al final (tarjeta 7, barra completa), avance monótono del `translateX` y del contador a lo largo del recorrido, mayor `--card-scale` en la tarjeta centrada frente a una alejada, ausencia de scroll horizontal inesperado de la página, recalculo correcto al cruzar el breakpoint de 900px con `resize`, clamp correcto ante un salto de scroll grande (scroll rápido), y equivalencia entre navegar hacia adelante y hacia atrás dentro del recorrido.
- **Quirk real del original preservado, no corregido**: la barra de progreso y el contador quedan recortados fuera del área visible por el auto-sizing de filas del grid CSS — verificado que la posición y los valores coinciden byte a byte con el oráculo; no es un defecto de esta migración, así que no se toca (regla nº1).
- **Bug de herramienta encontrado y corregido antes de comitear**: el test "navegación adelante y atrás" fallaba de forma intermitente (`07` en vez de `06`) al ejecutarse dentro de la suite completa pero nunca en aislado. Causa: no esperaba a que se asentara la secuencia de entrada (el `document.fonts.ready` del propio hook puede disparar una remedición tardía que desplaza las métricas de scroll bajo el mismo `scrollY` — mismo comportamiento que el original, no un defecto) antes de empezar su propia secuencia de scroll. Corregido esperando a que el preloader quede oculto más un margen fijo antes de iniciar el test.
- **Segundo bug de herramienta, de contención de recursos bajo ejecución paralela**: con 11 workers de Playwright compitiendo por CPU, los tests de paridad de píxel fallaban de forma intermitente (un test distinto en cada ejecución completa de la suite, siempre estable en aislado) con diferencias de 0.7–1.7%. Diagnosticado como una captura ocasional a mitad de la transición CSS de 180ms en `--card-scale`/`--image-scale` (el margen de espera post-scroll de 300ms no cubría con suficiente holgura esa transición bajo carga de CPU real). Corregido ampliando el margen a 600ms en el test y en el capturador de oráculo (regenerado con el mismo margen); confirmado estable en 5 ejecuciones completas seguidas de `projects-gallery.spec.ts` sin ningún fallo.
